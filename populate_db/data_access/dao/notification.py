    ...
    def create(self, data: NotificationDTO) -> None:
        ...
        try:
            self._db_gateway.cursor.execute(
                "INSERT INTO notifications (message, notification_type_id, user_id, created_at, updated_at) VALUES"
                " (%s, %s, %s, %s, %s);",
                (data.message, data.notification_type_id, data.user_id, data.created_at, data.updated_at),
            )
